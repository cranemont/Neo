...
export class Context {
  ...
  async submitFileChooser(paths: string[]) {
    if (!this._fileChooser) throw new Error('No file chooser visible');
  ...
  private async _createPage(): Promise<{ browser?: playwright.Browser; page: playwright.Page }> {
    if (process.env.PLAYWRIGHT_WS_ENDPOINT) {
      ...
      if (this._launchOptions) url.searchParams.set('launch-options', JSON.stringify(this._launchOptions));
